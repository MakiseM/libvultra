...
namespace vultra
{
    ...
    GeneralGaussianSplatFeature::GeneralGaussianSplatFeature()
    {
        ...
        m_TemporalFilterPass    = new GeneralGaussianSplatTemporalFilterPass();
    }

    GeneralGaussianSplatFeature::~GeneralGaussianSplatFeature()
    {
        delete m_FoveatedCompositePass;
        delete m_TemporalFilterPass;
        delete m_RenderPass;
    ...
    void GeneralGaussianSplatFeature::addPasses(FrameGraphBuildContext& ctx)
    {
        ...
        auto setFinalColor = [&](FrameGraphResource color) {
            if (color)
            {
                if (m_TemporalFilterPass)
                    color = m_TemporalFilterPass->filter(ctx, color);
