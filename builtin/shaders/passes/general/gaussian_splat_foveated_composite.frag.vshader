...
vec4 maxAlphaLayer(const vec4 fovea, const vec4 mid, const vec4 outer)
{
    vec4 best = fovea;
    if (mid.a > best.a)
        best = mid;
...
float layerLuminance(const vec3 color)
{
...
void main()
{
    ...
    vec4 gaussian = mix(mix(fovea, mid, layerBlend.x), outer, layerBlend.y);
    const vec4 fallbackLayer = maxAlphaLayer(fovea, mid, outer);
    const float maxAlpha = fallbackLayer.a;

    // Coverage-safe transition fallback: prevent transparent black layers from
    // darkening a ring seam, but keep true all-layer alpha holes visible.
    const float alphaDrop = maxAlpha - gaussian.a;
    const float significantAlphaDrop = max(1e-3, 0.10 * maxAlpha);
    const float gaussianLuma = layerLuminance(gaussian.rgb);
    const float fallbackLuma = layerLuminance(fallbackLayer.rgb);
    const bool alphaSeam = alphaDrop > significantAlphaDrop;
    const bool rgbSeam = alphaDrop > 1e-4 && fallbackLuma > 1e-4 && gaussianLuma < 0.5 * fallbackLuma;
    if (maxAlpha > 1e-4 && (alphaSeam || rgbSeam))
        gaussian = fallbackLayer;
